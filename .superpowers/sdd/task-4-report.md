...
- Player mapping uses `OrdinalIgnoreCase` both for Riot ID uniqueness and active-player matching. IDs that differ only by case are rejected as duplicate protocol data before local-player lookup. Mapping requires exactly ten unique players, a uniquely identified active player, exactly five players per explicit `ORDER`/`CHAOS` side, and exactly five enemies. Hidden identities and known-but-unavailable structures fail as `ParticipantsUnavailableException`; empty or unknown team enum values fail as `ProtocolChangedException`.
...
- `dotnet test -c Release --no-restore --filter LeagueSessionTests`
  - PASS: 21, FAIL: 0, SKIP: 0. This includes concurrent TLS isolation and case-insensitive duplicate-identity coverage.
- `dotnet test -c Release --no-restore`
  - Core PASS: 11, FAIL: 0.
  - Infrastructure PASS: 29, FAIL: 0.
